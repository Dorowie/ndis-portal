...
// HIIIII SO CHANGE THE EAMIL AND PASSWORD EVRY TEST THANK YOU
...
test('Show validation error on missing fields', async ({ page }) => {
  await page.goto(`${BASE_URL}/login?returnUrl=%2Fdashboard`);
  ...
  // 5. Login with valid participant account
test('Login successfully with valid participant account', async ({ page }) => {
  ...
  }).fill('participant_new@example.com');
  ...
  await expect(
    page.getByText(/dashboard|welcome|services/i)
  ).toBeVisible();
});



  // 6. Login with valid coordinator account
test('Login successfully with valid coordinator account', async ({ page }) => {
  ...
  await expect(page.getByText(/dashboard|welcome|services/i)).toBeVisible();
});
...
// 10. Wrong password for participant/coordinator
